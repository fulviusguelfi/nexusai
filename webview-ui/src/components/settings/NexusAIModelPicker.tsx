...
import { type NexusAIRecommendedModel, NexusAIRecommendedModelsResponse } from "@shared/proto/nexusai/models"
...
export interface NexusAIModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
const NEXUSAI_RECOMMENDED_MODELS_RETRY_DELAY_MS = 5000
...
function toFeaturedModelCardEntry(
	model: Pick<NexusAIRecommendedModel, "id" | "description" | "tags">,
...
const RECOMMENDED_MODELS_FALLBACK: FeaturedModelCardEntry[] = NEXUSAI_RECOMMENDED_MODELS_FALLBACK.recommended
	.map((model) => toFeaturedModelCardEntry(model, "RECOMMENDED"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const FREE_MODELS_FALLBACK: FeaturedModelCardEntry[] = NEXUSAI_RECOMMENDED_MODELS_FALLBACK.free
	.map((model) => toFeaturedModelCardEntry(model, "FREE"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const NexusAIModelPicker: React.FC<NexusAIModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	...
	const refreshClineRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			const response = await ModelsServiceClient.makeUnaryRequest(
				...
				EmptyRequest.create({}),
				EmptyRequest.toJSON,
				NexusAIRecommendedModelsResponse.fromJSON,
			)
	...
	const fetchClineRecommendedModels = useCallback(async () => {
		...
		if (clineRecommendedModelsRetryTimeoutRef.current === null) {
			...
			}, NEXUSAI_RECOMMENDED_MODELS_RETRY_DELAY_MS)
		}
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				...
				<DropdownWrapper ref={dropdownRef}>
					<VSCodeTextField
						...
						style={{
							width: "100%",
							zIndex: NEXUSAI_MODEL_PICKER_Z_INDEX,
							position: "relative",
						}}
						value={searchTerm}>
						{searchTerm && (
...
export default NexusAIModelPicker
...
const NEXUSAI_MODEL_PICKER_Z_INDEX = 1_000
...
const DropdownList = styled.div`
	...
	z-index: ${NEXUSAI_MODEL_PICKER_Z_INDEX - 1};
