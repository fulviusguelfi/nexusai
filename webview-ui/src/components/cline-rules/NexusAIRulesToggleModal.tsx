...
} from "@shared/proto/nexusai/file"
...
const NexusAIRulesToggleModal: React.FC = () => {
	const {
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: NexusAIRulesToggles) => {
				if (response.toggles) {
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									...
									<VSCodeLink
										className="text-xs"
										href="https://docs.cline.bot/features/nexusai-rules"
...
export default NexusAIRulesToggleModal
