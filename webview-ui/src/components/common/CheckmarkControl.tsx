...
import { Int64Request } from "@shared/proto/nexusai/common"
...
export const CheckmarkControl = ({ messageTs, isCheckpointCheckedOut }: CheckmarkControlProps) => {
	...
	const handleRestoreTask = async () => {
		...
		try {
			const restoreType: NexusAICheckpointRestore = "task"
	...
	const handleRestoreWorkspace = async () => {
		...
		try {
			const restoreType: NexusAICheckpointRestore = "workspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
	...
	const handleRestoreBoth = async () => {
		...
		try {
			const restoreType: NexusAICheckpointRestore = "taskAndWorkspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
