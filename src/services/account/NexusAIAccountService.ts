...
export class NexusAIAccountService {
	private static instance: NexusAIAccountService
	...
	/**
	 * Returns the singleton instance of NexusAIAccountService
	 * @returns Singleton instance of NexusAIAccountService
	 */
	public static getInstance(): NexusAIAccountService {
		if (!NexusAIAccountService.instance) {
			NexusAIAccountService.instance = new NexusAIAccountService()
		}
		return NexusAIAccountService.instance
	}
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(NEXUSAI_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(NEXUSAI_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
