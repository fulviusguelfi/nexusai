...
export interface NexusAIStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: NexusAIStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and NexusAISyncStorage both satisfy this interface.
 */
export interface NexusAIMemento {
 ...
 */
export abstract class NexusAIStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "NexusAIStorage"
	/**
 ...
 * Unlike NexusAIStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: NexusAIStorageChangeEvent) => void

export abstract class NexusAISyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of NexusAIStorage using a Map.
 */
export class NexusAIInMemoryStorage extends NexusAIStorage {
	/**
