...
import { NexusAIStorage } from "./NexusAIStorage"
...
/**
 * S3/R2 blob storage implementation of NexusAIStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class NexusAIBlobStorage extends NexusAIStorage {
	override name = "NexusAIBlobStorage"

	private static store: NexusAIBlobStorage | null = null
	static get instance(): NexusAIBlobStorage {
		if (!NexusAIBlobStorage.store) {
			NexusAIBlobStorage.store = new NexusAIBlobStorage()
		}
		return NexusAIBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!NexusAIBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[NexusAIBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[NexusAIBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = NexusAIBlobStorage.instance
