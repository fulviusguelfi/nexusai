...
import { NexusAIEndpoint, NexusAIEnv } from "@/config"
...
describe("Controller Marketplace Filtering", () => {
	...
	// Initialize NexusAIEndpoint before tests run (required for NexusAIEnv.config() to work)
	before(async () => {
		if (!NexusAIEndpoint.isInitialized()) {
			await NexusAIEndpoint.initialize("/test/extension")
		}
	})
	...
	describe("API interaction", () => {
		it("should call marketplace API with correct parameters", async () => {
			...
			callArgs[0].should.equal(`${NexusAIEnv.config().mcpBaseUrl}/marketplace`)
		})
