...
export class NexusAIDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = NexusAIDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof NexusAIDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || NexusAIDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = NexusAIDataMock.findUserByToken(token)
