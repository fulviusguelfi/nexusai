 ...
 * The extension is launched with NEXUSAI_ENVIRONMENT=local so every outbound
...
e2e.describe("Cline Account — free model onboarding + chat (requests intercepted by mock server)", () => {
	...
	e2e(
		...
		async ({ sidebar, server }) => {
			...
			// "KwaiKAT: Kat Coder Pro" is the first model in NEXUSAI_ONBOARDING_MODELS.
			...
			//  In E2E mode (NEXUSAI_ENVIRONMENT=local + E2E_TEST=true) the mock
