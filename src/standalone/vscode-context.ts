...
log(`NEXUSAI_ENVIRONMENT: ${process.env.NEXUSAI_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(clineDir?: string) {
	const NEXUSAI_DIR = clineDir || process.env.NEXUSAI_DIR || `${os.homedir()}/.nexusai`
	const DATA_DIR = path.join(NEXUSAI_DIR, SETTINGS_SUBFOLDER)
