...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("nexusai.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("nexusai.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): NexusAIMessage[] {
