...
import { NexusAIAssistantThinkingBlock, NexusAIStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: NexusAIStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is NexusAIAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
