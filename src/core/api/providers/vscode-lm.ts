...
export class VsCodeLmHandler implements ApiHandler, SingleCompletionHandler {
	...
	constructor(options: VsCodeLmHandlerOptions) {
		...
		try {
			...
			throw new Error(
				`NexusAI <Language Model API>: Failed to initialize handler: ${error instanceof Error ? error.message : "Unknown error"}`,
			)
		}
	}
	...
	async createClient(selector: vscode.LanguageModelChatSelector): Promise<vscode.LanguageModelChat> {
		try {
					...
					return {
						stream: (async function* () {
							yield new vscode.LanguageModelTextPart(
								"Language model not available. Please check GitHub Copilot configuration.",
							)
						})(),
						text: (async function* () {
							yield "Language model not available. Please check GitHub Copilot configuration."
						})(),
					}
				},
				countTokens: async () => 0,
			...
			throw new Error(`NexusAI <Language Model API>: Failed to select model: ${errorMessage}`)
		}
	}
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		if (!this.client) {
			Logger.debug("NexusAI <Language Model API>: Getting client with options:", {
			...
			try {
				...
				Logger.debug("NexusAI <Language Model API>: Creating client with selector:", selector)
				this.client = await this.createClient(selector)
			} catch (error) {
				const message = error instanceof Error ? error.message : "Unknown error"
				Logger.error("NexusAI <Language Model API>: Client creation failed:", message)
				throw new Error(`NexusAI <Language Model API>: Failed to create client: ${message}`)
			}
		}
	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[]): ApiStream {
		...
		try {
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof vscode.LanguageModelTextPart) {
					...
					if (typeof chunk.value !== "string") {
						Logger.warn("NexusAI <Language Model API>: Invalid text part value received:", chunk.value)
						continue
					}
					...
					try {
						...
						if (!chunk.name || typeof chunk.name !== "string") {
							Logger.warn("NexusAI <Language Model API>: Invalid tool name received:", chunk.name)
							continue
						}

						if (!chunk.callId || typeof chunk.callId !== "string") {
							Logger.warn("NexusAI <Language Model API>: Invalid tool callId received:", chunk.callId)
							continue
						}

						// Ensure input is a valid object
						if (!chunk.input || typeof chunk.input !== "object") {
							Logger.warn("NexusAI <Language Model API>: Invalid tool input received:", chunk.input)
							continue
						}
						...
						Logger.debug("NexusAI <Language Model API>: Processing tool call:", {
							name: chunk.name,
							callId: chunk.callId,
						...
						Logger.error("NexusAI <Language Model API>: Failed to process tool call:", error)
					}
				} else {
					Logger.warn("NexusAI <Language Model API>: Unknown chunk type received:", chunk)
				}
			}
			...
			if (error instanceof vscode.CancellationError) {
				throw new Error("NexusAI <Language Model API>: Request cancelled by user")
			}

			if (error instanceof Error) {
				Logger.error("NexusAI <Language Model API>: Stream error details:", {
			...
			if (typeof error === "object" && error !== null) {
				...
				Logger.error("NexusAI <Language Model API>: Stream error object:", errorDetails)
				throw new Error(`NexusAI <Language Model API>: Response stream error: ${errorDetails}`)
			}
			// Fallback for unknown error types
			const errorMessage = String(error)
			Logger.error("NexusAI <Language Model API>: Unknown stream error:", errorMessage)
			throw new Error(`NexusAI <Language Model API>: Response stream error: ${errorMessage}`)
		}
	}
	...
	getModel(): { id: string; info: ModelInfo } {
		if (this.client) {
			...
			for (const [prop, value] of Object.entries(requiredProps)) {
				if (!value && value !== 0) {
					Logger.warn(`NexusAI <Language Model API>: Client missing ${prop} property`)
				}
			}
		...
		Logger.debug("NexusAI <Language Model API>: No client available, using fallback model info")
