...
import { McpServer, NEXUSAI_MCP_TOOL_IDENTIFIER } from "@/shared/mcp"
import { ModelFamily } from "@/shared/prompts"
import { NexusAIDefaultTool } from "@/shared/tools"
import { type NexusAIToolSpec, toolSpecFunctionDeclarations, toolSpecFunctionDefinition, toolSpecInputSchema } from "../spec"
import { PromptVariant, SystemPromptContext } from "../types"

export class NexusAIToolSet {
	// A list of tools mapped by model group
	private static variants: Map<ModelFamily, Set<NexusAIToolSet>> = new Map()

	private constructor(
		public readonly id: string,
		public readonly config: NexusAIToolSpec,
	) {
		this._register()
	}

	public static register(config: NexusAIToolSpec): NexusAIToolSet {
		return new NexusAIToolSet(config.id, config)
	}

	private _register(): void {
		const existingTools = NexusAIToolSet.variants.get(this.config.variant) || new Set()
		if (!Array.from(existingTools).some((t) => t.config.id === this.config.id)) {
			existingTools.add(this)
			NexusAIToolSet.variants.set(this.config.variant, existingTools)
		}
	}

	public static getTools(variant: ModelFamily): NexusAIToolSet[] {
		const toolsSet = NexusAIToolSet.variants.get(variant) || new Set()
		const defaultSet = NexusAIToolSet.variants.get(ModelFamily.GENERIC) || new Set()

		return toolsSet ? Array.from(toolsSet) : Array.from(defaultSet)
	}

	public static getRegisteredModelIds(): string[] {
		return Array.from(NexusAIToolSet.variants.keys())
	}

	public static getToolByName(toolName: string, variant: ModelFamily): NexusAIToolSet | undefined {
		const tools = NexusAIToolSet.getTools(variant)
	...
	public static getToolByNameWithFallback(toolName: string, variant: ModelFamily): NexusAIToolSet | undefined {
		// Try exact variant first
		const exact = NexusAIToolSet.getToolByName(toolName, variant)
		if (exact) {
			return exact
		}

		// Fallback to GENERIC
		const generic = NexusAIToolSet.getToolByName(toolName, ModelFamily.GENERIC)
		if (generic) {
			return generic
		}

		// Final fallback: search across all registered variants
		for (const [, tools] of NexusAIToolSet.variants) {
	...
	public static getToolsForVariantWithFallback(variant: ModelFamily, requestedIds: string[]): NexusAIToolSet[] {
		const resolved: NexusAIToolSet[] = []
		for (const id of requestedIds) {
			const tool = NexusAIToolSet.getToolByNameWithFallback(id, variant)
			if (tool) {
	...
	public static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): NexusAIToolSet[] {
		const resolved: NexusAIToolSet[] = []
		const requestedIds = variant.tools ? [...variant.tools] : []
		for (const id of requestedIds) {
			const tool = NexusAIToolSet.getToolByNameWithFallback(id, variant.family)
			if (tool) {
				// Avoid duplicates by id
				if (!resolved.some((t) => t.config.id === tool.config.id)) {
					resolved.push(tool)
	...
	private static getDynamicSubagentToolSpecs(variant: PromptVariant, context: SystemPromptContext): NexusAIToolSpec[] {
		if (context.subagentsEnabled !== true || context.isSubagentRun) {
			return []
		}

		const requestedIds = variant.tools ? [...variant.tools] : []
		const shouldIncludeSubagentTools = requestedIds.length === 0 || requestedIds.includes(NexusAIDefaultTool.USE_SUBAGENTS)
		...
		return agentConfigs.map(({ toolName, config }) => ({
			...
			id: NexusAIDefaultTool.USE_SUBAGENTS,
			name: toolName,
	...
	public static getEnabledToolSpecs(variant: PromptVariant, context: SystemPromptContext): NexusAIToolSpec[] {
		const registeredTools = NexusAIToolSet.getEnabledTools(variant, context).map((tool) => tool.config)
		const dynamicSubagentTools = NexusAIToolSet.getDynamicSubagentToolSpecs(variant, context)

		const includesDynamicSubagents = dynamicSubagentTools.length > 0
		const filteredRegistered = includesDynamicSubagents
			? registeredTools.filter((tool) => tool.id !== NexusAIDefaultTool.USE_SUBAGENTS)
			: registeredTools
	...
	public static getNativeTools(variant: PromptVariant, context: SystemPromptContext) {
		...
		// Base set
		const toolConfigs = NexusAIToolSet.getEnabledToolSpecs(variant, context)
		...
		const converter = NexusAIToolSet.getNativeConverter(context.providerInfo.providerId, context.providerInfo.model.id)
...
/**
 * Convert an MCP server's tools to NexusAIToolSpec format
 */
export function mcpToolToClineToolSpec(family: ModelFamily, server: McpServer): NexusAIToolSpec[] {
	...
	return tools
		.map((mcpTool) => {
			...
			const mcpToolName = server.uid + NEXUSAI_MCP_TOOL_IDENTIFIER + mcpTool.name
