...
export class AccessMcpResourceHandler implements IFullyManagedTool {
	readonly name = NexusAIDefaultTool.MCP_ACCESS
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!server_name) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(NexusAIDefaultTool.MCP_ACCESS, "server_name")
		}

		if (!uri) {
			config.taskState.consecutiveMistakeCount++
			return await config.callbacks.sayAndCreateMissingParamError(NexusAIDefaultTool.MCP_ACCESS, "uri")
		}
