...
import { NexusAISubagentUsageInfo } from "@shared/ExtensionMessage"
...
describe("SubagentToolHandler", () => {
	...
	it("returns missing parameter error when no prompts are provided", async () => {
		...
		const result = await handler.execute(config, {
			type: "tool_use",
			name: NexusAIDefaultTool.USE_SUBAGENTS,
			params: {},
			partial: false,
		})
	...
	it("fans out prompts in parallel and emits aggregated status", async () => {
		...
		const usagePayload = JSON.parse(usageCalls[0].args[1]) as NexusAISubagentUsageInfo
	...
	it("runs configured subagent tools using the prompt parameter", async () => {
		...
		const result = await handler.execute(config, {
			type: "tool_use",
			name: dynamicToolName as NexusAIDefaultTool,
	...
	it("requires prompt for configured subagent tools", async () => {
		...
		assert.equal(result, "missing")
		assert.equal(taskState.consecutiveMistakeCount, 1)
		sinon.assert.calledWithExactly(callbacks.sayAndCreateMissingParamError, NexusAIDefaultTool.USE_SUBAGENTS, "prompt")
	})
})
