...
import type { NexusAISayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified NexusAISayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: NexusAISayGenerateExplanation["status"],
	error?: string,
): string {
	const message: NexusAISayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = NexusAIDefaultTool.GENERATE_EXPLANATION
