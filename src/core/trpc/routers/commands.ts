...
export const commandsRouter = router({
	...
	explainWithCline: publicProcedure
		.input(z.custom())
		.mutation(({ ctx, input }) => explainWithCline(ctx.controller, input as any)),
	improveWithCline: publicProcedure
		.input(z.custom())
		.mutation(({ ctx, input }) => improveWithCline(ctx.controller, input as any)),
})
