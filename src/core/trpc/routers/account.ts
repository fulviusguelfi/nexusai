...
export const accountRouter = router({
	...
	authStateChanged: publicProcedure
		.input(z.custom())
		.mutation(({ ctx, input }) => authStateChanged(ctx.controller, input as any)),
	getUserCredits: publicProcedure.input(e).query(({ ctx, input }) => getUserCredits(ctx.controller, input)),
	getOrganizationCredits: publicProcedure
		.input(z.custom())
		.query(({ ctx, input }) => getOrganizationCredits(ctx.controller, input as any)),
	getUserOrganizations: publicProcedure.input(e).query(({ ctx, input }) => getUserOrganizations(ctx.controller, input)),
	setUserOrganization: publicProcedure
		.input(z.custom())
		.mutation(({ ctx, input }) => setUserOrganization(ctx.controller, input as any)),
	openrouterAuthClicked: publicProcedure.input(e).mutation(({ ctx, input }) => openrouterAuthClicked(ctx.controller, input)),
	requestyAuthClicked: publicProcedure
		.input(z.custom())
		.mutation(({ ctx, input }) => requestyAuthClicked(ctx.controller, input as any)),
