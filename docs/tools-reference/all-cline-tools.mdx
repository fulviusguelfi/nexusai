...
| `list_processes` | List running processes on the host (optional filter) |
| `kill_process` | Terminate a process by PID (requires user approval) |
...
Kill a process by PID (will prompt for approval):
```xml
<kill_process>
<pid>12345</pid>
